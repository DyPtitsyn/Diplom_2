...
class UserApi:
    ...
    @staticmethod
    @allure.step("POST запрос на авторизацию пользователя")
    ...
    @staticmethod
    @allure.step("PATCH запрос на изменение данных пользователя")
    def change_user_info(update_payload, access_token):
        return requests.patch(urls.BASE_URL + urls.CHANGE_USER_INFO_ENDPOINT, json=update_payload, headers={'Authorization': access_token})
